(function (root) {
  ...
  function createOverlay(payload) {
    ...
    title.textContent = "24場サイン 買い目入力 " + UI_VERSION;
    ...
    action.id = "manshu-autofill-start";
    action.type = "button";
    action.textContent = "入力開始";
    action.hidden = true;
    action.style.cssText = "width:100%;min-height:48px;margin-top:14px;border:0;border-radius:5px;background:#087f6b;color:#fff;font-weight:900;font-size:16px;";
  ...
  async function waitForHumanStart(overlay, cancelled) {
    overlay.action.hidden = false;
    overlay.action.textContent = "入力開始";
    overlay.setStatus(
      UI_VERSION + "を読み込みました",
      "このボタンを指で押すまでは公式画面を操作しません"
    );
    await new Promise(function (resolve, reject) {
      ...
      var cancelId;
      function cleanup() {
        overlay.action.removeEventListener("click", onClick, true);
      ...
      overlay.action.addEventListener("click", onClick, true);
      timeoutId = setTimeout(function () {
        settle(reject, new Error("入力開始が2分以内に押されませんでした"));
      }, 120000);
  ...
  async function run() {
    ...
    try {
      ...
      await waitForHumanStart(overlay, function () { return cancelled; });
  ...
  if (root) {
    root.__manshuAutofillApi = api;
    root.__manshuAutofillCoreLoaded = true;
  }
